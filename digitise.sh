...
if xrdfs $EOSSERVER stat $OUTPUTDIR/$OUTPUTFILE_DIGI; then
	if [ "${RESIMULATE:-False}" = "True" ]; then
		echo "WARNING: output $OUTPUTDIR/$OUTPUTFILE_DIGI already exists — deleting for resimulation."
		xrdfs $EOSSERVER rm $OUTPUTDIR/$OUTPUTFILE_DIGI
	else
